...
from panpe.nns.embedding_nn.conv_encoder import ConvEncoder
from panpe.nns.embedding_nn.resnets import ResidualNet
from panpe.nns.scalers import DataScaler, IdentityScaler
...
class EmbeddingNN(nn.Module):
    def __init__(
        ...
        phi_dim: int = 20,
        use_fft: bool = False,
        data_scaler: DataScaler = None,
        activation: str = "gelu",
        conv_activation: str = "lrelu",
        resnet_activation: str = "relu",
        cat_fixed_theta_mask: bool = False,
    ) -> None:
        super().__init__()
        ...
        self.data_scaler = data_scaler or IdentityScaler()

        self.fc = ResidualNet(
            latent_dim * num_kernel + phi_dim,
            latent_dim - phi_dim,
            hidden_features=2 * latent_dim,
        ...
        self.cat_fixed_theta_mask = cat_fixed_theta_mask

    def forward(self, x: dict[str, Tensor]) -> Tensor:
        x = self.data_scaler(x)
        q = x["q"]
        phi = x["phi"]
        fixed_theta_mask = x.get("fixed_theta_mask", None)

        if self.in_dim == 1:
            y = x["data"].unsqueeze(-1)
        else:
            y = stack([x["data"], x["sigmas"]], dim=-1)

        drop_mask = x.get("drop_mask", None)
        ...
        if self.cat_fixed_theta_mask:
            x = cat([x, fixed_theta_mask], dim=-1)
        return x
